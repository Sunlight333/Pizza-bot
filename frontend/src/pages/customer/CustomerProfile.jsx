...
import { profile as profileApi } from '@/services/customerApi'
  ...
  if (isLoading) {
    return (
      <div className="max-w-md mx-auto px-5 py-6 space-y-4">
        ...
        <div className="c-skeleton h-14 rounded-xl" />
        <div className="c-skeleton h-14 rounded-xl" />
      </div>
    )
  }
  ...
  return (
    <div className="max-w-md mx-auto px-5 py-6">
      ...
      <button
        ...
        className="c-card c-card-tap w-full mt-6 p-4 flex items-center justify-between"
      >
        <span className="flex items-center gap-3">
          <MapPin className="w-5 h-5" style={{ color: 'var(--c-ovenred)' }} />
          <span className="font-semibold">Endereços salvos</span>
        </span>
        <ChevronRight className="w-5 h-5" style={{ color: 'var(--c-slate-muted)' }} />
      </button>

      <button
        onClick={doLogout}
        className="w-full mt-6 py-3 flex items-center justify-center gap-2 text-[13px]"
