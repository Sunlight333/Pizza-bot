...
import { profile as profileApi, lookupCep } from '@/services/customerApi'
...
export default function CustomerAddresses() {
  ...
  function startNew() { setEditing(-1); setDraft({ ...EMPTY }) }
  function startEdit(idx) { setEditing(idx); setDraft({ ...EMPTY, ...(data.addresses[idx] || {}) }) }
  ...
  async function setDefault(idx) {
    try { await profileApi.addresses.setDefault(idx); toast.success('Endereço padrão atualizado'); refetch() }
    catch (e) { toast.error(e?.message || 'Erro') }
  }

  async function remove(idx) {
    if (!confirm('Remover este endereço?')) return
    try { await profileApi.addresses.remove(idx); toast.success('Endereço removido'); refetch() }
  ...
  return (
    <div className="max-w-md mx-auto px-5 py-6">
      <h1 className="font-display text-3xl mb-6">Endereços</h1>

      <div className="space-y-3">
        {data.addresses.map((a, i) => (
          <div key={i} className="c-card p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {data.default_index === i && (
                    <span className="text-[11px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded"
                          style={{ color: 'var(--c-ovenred)', background: 'rgba(139,26,26,0.10)' }}>
                      Padrão
                    </span>
                  )}
                </div>
                ...
                <p className="text-[13px]" style={{ color: 'var(--c-slate-muted)' }}>{a.neighborhood}</p>
              </div>
