"""
Pichya operational seed — applies the customer's actual operating data
to the singleton BotConfig and the delivery_zones table.

Run with: docker compose exec backend python -m app.seed_pichya
...
# Pichya: Tuesday → Sunday, 17:00–23:00 (closed Monday).
...
PICHYA_HOURS_START = 17
PICHYA_HOURS_END = 23
PICHYA_CLOSED_WEEKDAYS = [0]  # Mon=0..Sun=6 (Python weekday convention)
...
PICHYA_BOT_NAME = "Bia"
PICHYA_GREETING = "Oi! Tudo bem? 😊 Em que posso te ajudar hoje?"
...
PICHYA_PIX_KEY = "24853133000179"
PICHYA_PIX_HOLDER = "Guilherme Guimaraes Lopes"
...
async def upsert_bot_config(db) -> None:
    ...
    cfg.bot_name = PICHYA_BOT_NAME
    cfg.greeting = PICHYA_GREETING
    cfg.working_hours_start = PICHYA_HOURS_START
    cfg.working_hours_end = PICHYA_HOURS_END
    cfg.closed_weekdays = list(PICHYA_CLOSED_WEEKDAYS)
    cfg.pix_key = PICHYA_PIX_KEY
    cfg.pix_holder = PICHYA_PIX_HOLDER
...
async def upsert_distance_bands(db) -> None:
    ...
    for label, kmin, kmax, fee, mins in PICHYA_DELIVERY_BANDS:
